...
# ╔═╡ 1558a1e1-627d-4356-8bdf-2360c8a577b9
using OrdinaryDiffEqTsit5

# ╔═╡ e6c64c80-773b-11ef-2379-bf6609137e69
md"""
# 1.3 Example: projectile problem

We have calculated the first-order expansion

$$\begin{equation*}
  y
  =
  y_0 + \epsilon y_1 + \mathcal{O}(\epsilon^2)
  =
  \tau - \frac{\tau^2}{2}
  + \epsilon \frac{\tau^3}{3} \left( 1 - \frac{\tau}{4} \right)
  + \mathcal{O}(\epsilon^2)
\end{equation*}$$

of the non-dimensionalized projectile problem

$$\begin{equation*}
  y''
  =
  - \frac{R^2}{(y x_\mathrm{ref} + R)^2}
  =
  - \frac{1}{(\epsilon y + 1)^2},
  \qquad
  y(0) = 0,
  \quad
  y'(0) = 1,
...
with $\epsilon = v_0^2 / (R g) \ll 1$. Here, we compare a numerical solution of the full problem with the zeroth- and first-order asymptotic expansion solutions.
"""

# ╔═╡ 8b2539ae-1078-44fc-9374-684920042c22
md"""
``\varepsilon`` = $(@bind epsilon Slider(range(0, 0.5, length = 51), default = 0.5, show_value = true))
"""

# ╔═╡ 8c924eba-78f4-4a97-829c-71ce98b2e164
function plot_solutions(epsilon)
	tspan = (0.0, 2.0)
	ode = SecondOrderODEProblem{false}(1.0, 0.0, tspan) do dy, y, p, tau
		return -1 / (1 + epsilon * y)^2
	end
	sol = solve(ode, Tsit5())

	fig = Figure()
	ax = Axis(fig[1, 1];
			  xlabel = L"Non-dimensionalized time $\tau$",
			  ylabel = L"Non-dimensionalized coordinate $y$")

	tau = range(tspan..., length = 500)

	y0 = @. tau - 0.5 * tau^2
	y1 = @. y0 + epsilon * tau^3 / 3 * (1 - 0.25 * tau)
	lines!(ax, tau, y0; label = L"y_0")
	lines!(ax, tau, y1; label = L"y_0 + \varepsilon y_1")
	lines!(ax, tau, (last∘sol).(tau); label = "numerical solution")
	
	axislegend(ax; position = :lt)
	fig
end

# ╔═╡ a7b52e73-e26d-4dc5-8bd9-e9a071e1351a
plot_solutions(epsilon)
...
manifest_format = "2.0"
project_hash = "3dfb069aa68b0bbb3e89add22ba05830767fd76e"
