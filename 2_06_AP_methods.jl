...
# ╔═╡ 0c7684a2-c933-47aa-ab96-dbaf4b33b5ea
begin
	using PlutoUI
	using PlutoUI: Slider
end

# ╔═╡ 2ee389ad-3905-4745-ae4c-fa880485b631
using LaTeXStrings

# ╔═╡ 0611368b-0f12-47f9-9369-c841f831b9c3
begin
	using CairoMakie
...
Consider Kaps' problem

$$\begin{equation*}
\begin{aligned}
  u_1'(t) &= u_2 - u_1 - u_1^2, \\
  u_2'(t) &= -2 u_2 + (u_1^2 - u_2) / \varepsilon
\end{aligned}
\end{equation*}$$

with $0 < \varepsilon \ll 1$. Next, we split the right-hand side into a nonstiff part (without $\varepsilon$) and a stiff part (involving $\varepsilon$) and compute numerical solutions using several schemes.
"""

# ╔═╡ 9fa6033c-d11e-4deb-8bf4-fc5decdac79b
...
# ╔═╡ 820375e4-a454-4d62-a65e-7ffd58dc718e
...
# ╔═╡ d9bbd7b2-df98-4a54-aaff-d7566e64000f
md"""
Initial condition ``u^0``: $(@bind u0 Select([
	[1.5, 1.0],
	[1.5, 2.25],
]))
"""

# ╔═╡ 9742bbd0-27f9-4ac2-b65b-bff052855967
md"""
``\varepsilon`` = $(@bind epsilon Slider(
	[1.0e-10, 1.0e-9, 1.0e-8, 1.0e-7, 1.0e-6, 1.0e-5, 1.0e-4],
	default = 1.0e-4, show_value = true))
``\Delta t`` = $(@bind dt Slider(
	[2.5e-3, 5.0e-3, 1.0e-2, 2.0e-2],
	default = 0.02, show_value = true))
"""

# ╔═╡ c3ac0c56-5b0d-4b48-87cf-cb3be18acc81
function plot_kaps(alg, epsilon, u0, dt)
	tspan = (0.0, 1.0)
	parameters = (; epsilon)
	ode = SplitODEProblem{true}(rhs_stiff!, rhs_nonstiff!, u0, tspan, parameters)
	integrator = Theseus.init(ode, alg; dt)
	...
	while !integrator.finalstep
		...
		push!(t, integrator.t)
		push!(u1, integrator.u[1])
		push!(u2, integrator.u[2])
    end

	fig = Figure()
	
	ax1 = Axis(fig[1, 1]; xlabel = L"Time $t$", ylabel = L"u_1")
	lines!(ax1, t, u1; label = "numerical")
	u1_limit = @. exp(-t) * u0[1]
	lines!(ax1, t, u1_limit; label = L"analytical limit $\varepsilon \to 0$")
	axislegend(ax1)
	
	ax2 = Axis(fig[1, 2]; xlabel = L"Time $t$", ylabel = L"u_2")
	lines!(ax2, t, u2; label = "numerical")
	u2_limit = @. u1_limit^2
	lines!(ax2, t, u2_limit; label = L"analytical limit $\varepsilon \to 0$")
	# axislegend(ax2)
...
function show_imex_tableaux_latex(a_ex, b_ex, c_ex,
								  a_im, b_im, c_im;
	                              digits = 6)
	function tex(x)
		if x isa Rational
			...
			return "\\frac{$(numerator(x))}{$(denominator(x))}"
		elseif x isa Integer
			return string(x)
		elseif x isa AbstractFloat
			y = round(x; sigdigits = digits)
			return isinteger(y) ? string(Int(y)) : string(y)
		else
			return string(x)
		end
	end
	...
	function tableau(A, b, c)
		s, m = size(A)
		@assert length(c) == s "Length of c must match number of rows of A."
		@assert length(b) == m "Length of b must match number of columns of A."

		colspec = "c|" * repeat("c", m)

		A_rows = String[]
		for i in 1:s
			push!(A_rows, join([tex(c[i]); tex.(A[i, :])], " & "))
		end
	...
	latex = """
	\\begin{equation*}
	$(tableau(a_ex, b_ex, c_ex))
	\\qquad\\qquad\\qquad
	$(tableau(a_im, b_im, c_im))
	\\end{equation*}
	"""

	return Markdown.MD(Markdown.LaTeX(latex))
end

# ╔═╡ b661a9dd-5a93-4cb5-bdde-b9d5a5f1319c
let
	(; a_ex, b_ex, c_ex, a_im, b_im, c_im) = Theseus.RKTableau(alg, Rational{Int})
...
function plot_convergence_rates(alg)
	...
	# function compute_errors(alg, u0, epsilon, dt)
	# 	tspan = (0.0, 5.0)
	# 	ode = SplitODEProblem{true}(stiff!, nonstiff!, u0, tspan, (; epsilon))
	# 	integrator1 = Theseus.init(ode, alg; dt)
	# 	integrator2 = Theseus.init(ode, alg; dt = dt / 2)
	# 	integrator3 = Theseus.init(ode, alg; dt = dt / 4)
		
	# 	err1a = 0.0 # abs(integrator.u[1] - acos(tanh(integrator.t)))
	# 	err2a = 0.0 # abs(integrator.u[2] - sin(acos(tanh(integrator.t))))
	# 	err1b = 0.0
	# 	err2b = 0.0
	# 	while !(integrator1.finalstep || 
	# 			integrator2.finalstep || 
	# 			integrator3.finalstep)
	#         Theseus.step!(integrator1)
			
	#         Theseus.step!(integrator2)
	#         Theseus.step!(integrator2)
			
	#         Theseus.step!(integrator3)
	#         Theseus.step!(integrator3)
	#         Theseus.step!(integrator3)
	#         Theseus.step!(integrator3)
			
	# 		err1a = max(err1a, abs(integrator1.u[1] - integrator2.u[1]))
	# 		err2a = max(err2a, abs(integrator1.u[2] - integrator2.u[2]))
			
	# 		err1b = max(err1b, abs(integrator2.u[1] - integrator3.u[1]))
	# 		err2b = max(err2b, abs(integrator2.u[2] - integrator3.u[2]))
	#     end
	# 	return err1a, err2a, err1b, err2b
	# end
	function compute_errors(alg, u0, epsilon, dt)
	    tspan = (0.0, 5.0)
	    ode = SplitODEProblem{true}(stiff!, nonstiff!, u0, tspan, (; epsilon))
	
	    integ_h  = Theseus.init(ode, alg; dt = dt)
	    integ_h2 = Theseus.init(ode, alg; dt = dt / 2)
	    integ_h4 = Theseus.init(ode, alg; dt = dt / 4)
	
	    num1a = num2a = den1a = den2a = 0.0
	    num1b = num2b = den1b = den2b = 0.0
	
	    while !(integ_h.finalstep || 
				integ_h2.finalstep || 
				integ_h4.finalstep)
	        Theseus.step!(integ_h)
	
	        Theseus.step!(integ_h2)
	        Theseus.step!(integ_h2)
	
	        Theseus.step!(integ_h4)
	        Theseus.step!(integ_h4)
	        Theseus.step!(integ_h4)
	        Theseus.step!(integ_h4)
	
	        # E_h: compare h against h/2
	        num1a += abs2(integ_h.u[1] - integ_h2.u[1])
	        num2a += abs2(integ_h.u[2] - integ_h2.u[2])
	        den1a += abs2(integ_h2.u[1])
	        den2a += abs2(integ_h2.u[2])
	
	        # E_{h/2}: compare h/2 against h/4
	        num1b += abs2(integ_h2.u[1] - integ_h4.u[1])
	        num2b += abs2(integ_h2.u[2] - integ_h4.u[2])
	        den1b += abs2(integ_h4.u[1])
	        den2b += abs2(integ_h4.u[2])
	    end
	
	    err1a = sqrt(num1a / den1a)
	    err2a = sqrt(num2a / den2a)
	    err1b = sqrt(num1b / den1b)
	    err2b = sqrt(num2b / den2b)
...
[[deps.Theseus]]
deps = ["Ariadne", "DiffEqBase", "Krylov", "LinearAlgebra", "SciMLBase", "UnPack"]
git-tree-sha1 = "fd7bd30f25acd8a6eaa2556f0551006857eb5327"
