...
# ╔═╡ bbf13e76-6510-479c-b120-a95b73e4e067
using LinearAlgebra

# ╔═╡ 9a6160ed-cda3-4bfe-9213-ff1d6b16f922
using OrdinaryDiffEqRosenbrock

# ╔═╡ e6c64c80-773b-11ef-2379-bf6609137e69
md"""
# 1.5 Singularly perturbed ODEs
...
u_1'(t) &= u_2(t),
\\
\varepsilon u_2'(t) &= (1 - u_1(t)^2) u_2(t) - u_1(t),
\end{aligned}
\end{equation*}$$

with initial conditions $u_1(0) = 1$, $u_2(0) = 1$. We compute numerical solutions for different values of $\varepsilon$.
"""

# ╔═╡ 8b2539ae-1078-44fc-9374-684920042c22
md"""
``\varepsilon`` = $(@bind epsilon Slider(
	range(0, 1, length = 100_001)[2:end],
	default = 1, show_value = true))
"""

# ╔═╡ 1b10cc8d-ba91-4d85-b5f8-01ffa1d992f3
function plot_van_der_pol(epsilon)
	u0 = [1.0, 1.0]
	tspan = (0.0, 5.0)
	alg = Rodas4()
	...
	ode_mass_matrix = Diagonal([1.0, epsilon])
	ode_rhs = ODEFunction(vanderpol; mass_matrix = ode_mass_matrix)
	ode = ODEProblem(ode_rhs, u0, tspan)
	ode_sol = solve(ode, alg)

	fig = Figure()
	ax = Axis(fig[1, 1]; xlabel = L"Time $t$", ylabel = L"u(t)")
	t = range(tspan..., length = 1000)
	lines!(ax, (first∘ode_sol).(t); label = L"ODE solution with $\varepsilon = %$epsilon$")
	ylims!(ax, -2.1, 2.1)

	return fig
end

# ╔═╡ bd1da88a-ae2a-41e5-8ca6-80cafb17f120
plot_van_der_pol(epsilon)
...
CairoMakie = "~0.15.8"
LaTeXStrings = "~1.4.0"
OrdinaryDiffEqRosenbrock = "~1.26.0"
PlutoUI = "~0.7.79"
"""
...
manifest_format = "2.0"
project_hash = "4710a392238059dc6c767382eca06a709491c4de"
