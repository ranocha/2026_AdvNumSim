...
# ╔═╡ 37b25cb6-2fc4-4261-a99e-d618f4b65de9
using LinearAlgebra
...
# ╔═╡ 95af5d9d-1fa5-468a-8ae6-844802fea444
using OrdinaryDiffEqSDIRK

# ╔═╡ 25ee1eef-9221-4b72-9553-f73f757de347
using OrdinaryDiffEqFIRK

# ╔═╡ 6052deab-3785-401a-93f9-92544f6c927c
using ODEInterfaceDiffEq

# ╔═╡ c1b99193-c55c-4b9d-a306-c4722148e7dd
using GeometricIntegrators: GeometricIntegrators

# ╔═╡ e6c64c80-773b-11ef-2379-bf6609137e69
md"""
# 2.16 Implementing DIRK methods

We compare two implementations of DIRK methods for linear ODEs

$$u'(t) = L u(t).$$

In the simple variant, we use the stage equations

$$y^i - h a_{ii} f(y^i) = u^n + h \sum_{j=1}^{i-1} a_{ij} f(y^j)$$

directly and insert the numerical solution $y^i$ into the right-hand side.
In the improved variant, we formulate the stage equations using the update

$$z^i = y^i - u^n$$

and avoid evaluating the right-hand side by reformulating the stage equations appropriately.
"""

# ╔═╡ 11272761-070d-4a83-a7b5-17e533f88b05
md"""
Next, we compare the results for the (singularly-perturbed) linear ODE (Example 1.2 from the lecture)

$$\varepsilon u''(t) + (1 + \varepsilon) u'(t) + u(t) = 0.$$

First, we reformulate the ODE as first-order system:

$$\begin{aligned}
u_1'(t) &= u_2'(t), \\
\varepsilon u_2'(t) &= - u_1(t) - (1 + \varepsilon) u_2(t).
\end{aligned}$$

For $$\varepsilon \to 0$$, we obtain the DAE
...
The algebraic equation can be solved uniquely for $u_2$ and leads to the reduced ODE
...
results in the analytical solution $u(t) = \exp(-t)$ for all $\varepsilon$.
"""

# ╔═╡ 4b72d94d-370b-4629-a274-104ceef1601c
md"""
``\Delta t`` = $(@bind dt_well_prepared Slider([1.0e-4, 1.0e-3, 1.0e-2, 1.0e-1], default = 1.0e-3, show_value = true))
"""

# ╔═╡ 70b7f616-9f92-4add-9115-e0f9b6487e8c
function compute_final_error_well_prepared(solve, alg, dt, ε)
	u0 = [1.0, -1.0]
	rhs_operator = [0 1; -1/ε -1-1/ε]
	tspan = (0.0, 2.0)
	sol = solve(rhs_operator, u0, tspan, alg; dt)
	u_ana = exp(-tspan[end]) * u0[1]
	return norm(sol.u[end] - [u_ana, -u_ana])
	# return abs(sol.u[end][1] - u_ana)
end

# ╔═╡ d570ecdf-d5ab-4753-95e3-240e459674d5
md"""
We also see that these methods are able to handle the stiff case and the transition to the limiting DAE quite well.
We will analyze these properties in the following lectures.
"""

# ╔═╡ 13a23f41-d9d0-4850-b344-fc8a73810cbb
md"""
## Performance of implementations of fully implicit Runge-Kutta methods

Before proceeding with analyzing time integration methods, we take a look at the performance of some implementations of fully implicit Runge-Kutta methods implemented in [GeometricIntegrators.jl](https://github.com/JuliaGNI/GeometricIntegrators.jl) and [OrdinaryDiffEq.jl](https://github.com/SciML/OrdinaryDiffEq.jl).
"""

# ╔═╡ 0982f6b3-447e-4f84-9d50-f654d766e75d
function compare_performance(n,
							 alg1 = RadauIIA5(),
							 alg2 = GeometricIntegrators.IRK(GeometricIntegrators.RadauIIA(3)))
	# Set up an ODE with n variables
	u0 = randn(n)
	tspan = (0.0, 10.0)
	dt = 0.5
	
	ode1 = ODEProblem{true}(u0, tspan) do du, u, p, t
		@. du = -u
		return nothing
	end
	solve(ode1, alg1; adaptive = false, dt)

	ode2 = GeometricIntegrators.ODEProblem(tspan, dt, u0) do du, t, u, p
		@. du = -u
		return nothing
	end
	GeometricIntegrators.integrate(ode2, alg2)
	
	t1 = @elapsed solve(ode1, RadauIIA5(); adaptive = false, dt)
	t2 = @elapsed GeometricIntegrators.integrate(ode2, alg2)

	return t1, t2
end

# ╔═╡ 073d7ca9-dd8a-461f-b4cf-b4dc2da36441
function plot_performance(alg1, alg2, title)
	n = [5, 10, 20, 40, 80, 160]
	t1 = zeros(length(n))
	t2 = zeros(length(n))
	for i in eachindex(n)
		t1[i], t2[i] = compare_performance(n[i], alg1, alg2)
	end
	
	fig = Figure()
	ax = Axis(fig[1, 1];
			  title,
			  xlabel = L"Size $M$", ylabel = "Runtime [s]",
			  xscale = log10, yscale = log10)
	scatterlines!(ax, n, t1; label = "OrdinaryDiffEq.jl")
	scatterlines!(ax, n, t2; label = "GeometricIntegrators.jl")
	axislegend(ax; position = :lt)
	return fig
end

# ╔═╡ 10efd8cd-2024-4a40-8d6b-83da8c03cca1
let
	alg1 = ImplicitMidpoint()
	alg2 = GeometricIntegrators.ImplicitMidpoint()
	title = "Implicit midpoint method, one stage, order 2"
	plot_performance(alg1, alg2, title)
end

# ╔═╡ 72d971b7-fe4e-457c-b2f2-503a5d23c779
let
	alg1 = RadauIIA3()
	alg2 = GeometricIntegrators.IRK(GeometricIntegrators.RadauIIA(2))
	title = "Radau-IIA, two stages, order 3"
	plot_performance(alg1, alg2, title)
end

# ╔═╡ 2ed3df9b-8954-4d3a-9705-194baf5e04b1
let
	alg1 = RadauIIA5()
	alg2 = GeometricIntegrators.IRK(GeometricIntegrators.RadauIIA(3))
	title = "Radau-IIA, three stages, order 5"
	plot_performance(alg1, alg2, title)
end

# ╔═╡ 82768ed1-6f1f-4ce2-8282-74db653302ac
let
	alg1 = RadauIIA9()
	alg2 = GeometricIntegrators.IRK(GeometricIntegrators.RadauIIA(5))
	title = "Radau-IIA, five stages, order 9"
	plot_performance(alg1, alg2, title)
end
...
# ╔═╡ da520a4e-c8f2-4295-bc13-4d0a7971e2d8
begin
	# Implicit part of the third-order method of Ascher, Ruuth, Spiteri (1997)
	# https://doi.org/10.1016/S0168-9274(97)00056-1
	struct ARS443 end

	function coefficients(alg::ARS443)
		RealT = Float64
		nstage = 5
...
# ╔═╡ ea5cc43b-eb3b-4915-a545-0a0447738782
begin
	# Implicit part of another third-order method of Ascher, Ruuth, Spiteri (1997)
	# https://doi.org/10.1016/S0168-9274(97)00056-1
	struct ARS233 end

	function coefficients(alg::ARS233)
		RealT = Float64
		nstage = 3
		gamma = (3 + sqrt(3)) / 6
	    a_im = zeros(RealT, nstage, nstage)
...
# ╔═╡ acf61f43-9bab-4424-ae09-38042ecaea32
begin
	# Fifth-order SDIRK method listed in Table IV.6.5 of Hairer, Wanner (2010)
	# https://doi.org/10.1007/978-3-642-05221-7
	struct SDIRK45 end

	function coefficients(alg::SDIRK45)
		A = [1/4 0 0 0 0;
			 1/2 1/4 0 0 0;
			 17/50 -1/25 1/4 0 0;
...
# ╔═╡ dc9526b9-e745-4054-ba3b-7369c2b7cbae
function solve_dirk_simple(rhs_operator, u0, tspan, alg;
                           dt,
                           save_everystep = false)
    A, b, c = coefficients(alg)
    s = length(b)
    ...
    y = similar(u) # stage value
    t = first(tspan)
    tmp = similar(u)
    k = Vector{typeof(u)}(undef, s) # stage derivatives
    for i in 1:s
        k[i] = similar(u)
    end
    ...
    while t < last(tspan)
        ...
        for i in 1:s
            ...
            # The right-hand side of the linear system formulated using the
			# stages y_i is
            @. tmp = u + dt * tmp
            ...
            if iszero(A[i, i])
                ...
                factor = A[i, i] * dt
				W = I - factor * rhs_operator
				y .= W \ tmp
            end

            # Compute new stage derivatives
            mul!(k[i], rhs_operator, y)
        end
...
# ╔═╡ e61a75fd-93df-46f5-98ed-a17b75b6e2fe
function solve_dirk_update(rhs_operator, u0, tspan, alg;
                           dt,
                           save_everystep = false)
    A, b, c = coefficients(alg)
    s = length(b)
    ...
    y = similar(u) # stage value
    z = similar(u) # stage update value
    t = first(tspan)
    tmp = similar(u)
    k_u = similar(u) # derivative of the previous state
    k = Vector{typeof(u)}(undef, s) # stage derivatives
    for i in 1:s
        k[i] = similar(u)
    end
    ...
    while t < last(tspan)
        ...
        # The "simple" one is
        #   y_i = u + h \sum_{j=1}^{i} a_{ij} f(y_j)
        # However, it can be better to use the smaller values
        #   z_i = (y_i - u) / h
        # so that the stage equations become
        #   u + h z_i = u + h \sum_{j=1}^{i} a_{ij} f(u + h z_j)
        # ⟺
        #   z_i - a_{ii} f(u + h z_i) = \sum_{j=1}^{i-1} a_{ij} f(u + h z_j)
        # For a linear problem f(u) = L u, this becomes
        #   (I - h a_{ii} L) z_i = a_{ii} L u + \sum_{j=1}^{i-1} a_{ij} L(u + h z_j)
        # We use this formulation and also avoid evaluating the operator L at
        # the numerical solutions (due to the large Lipschitz constant), but instead
        # rearrange the equation to obtain the required RHS values as
        #   L(u + h z_i) = a_{ii}^{-1} (z_i - \sum_{j=1}^{i-1} a_{ij} L(u + h z_j))
        mul!(k_u, rhs_operator, u)
        ...
        for i in 1:s
            ...
            # The right-hand side of the linear system formulated using the
			# stages y_i instead of the stage updates z_i would be
            #   @. tmp = u + dt * tmp
            # By using the stage updates z_i, we avoid the possibly different
			# scales for small dt.
            @. tmp = A[i, i] * k_u + tmp
            ...
            if iszero(A[i, i])
				...
				# For an efficient implementation, we would store the factorization
				# and re-use it for several stages if possible.
				W = I - factor * rhs_operator
				z .= W \ tmp
            end
            ...
            if iszero(A[i, i])
                ...
                # The code below is equivalent (on paper) to
                #   mul!(k_stiff[i], rhs_operator, y)
                # but avoids evaluating the operator at the numerical solution.
                @. tmp = z
                for j in 1:(i - 1)
                    @. tmp = tmp - A[i, j] * k[j]
                end
...
# ╔═╡ 4673b9fb-f98b-4ca3-86da-4b8b15c7e1df
md"""
`alg` = $(@bind alg_well_prepared Select([SDIRK45() => "SDIRK45", ARS443() => "ARS443", ARS233() => "ARS233"]))
"""

# ╔═╡ a80b078c-3016-461c-a04e-9eb3e165a3fe
let dt = dt_well_prepared, alg = alg_well_prepared
	fig = Figure()
	ax = Axis(fig[1, 1];
			  xlabel = L"\varepsilon", ylabel = "Error at the final time",
			  xscale = log10, yscale = log10)

	ε = 10.0 .^ (-15:0)
	
	err = map(ε) do ε
		compute_final_error_well_prepared(solve_dirk_simple, alg, dt, ε)
	end
	scatter!(ax, ε, err; label = "simple formulation")
	
	err = map(ε) do ε
		compute_final_error_well_prepared(solve_dirk_update, alg, dt, ε)
	end
	scatter!(ax, ε, err; label = "update formulation")
...
[compat]
CairoMakie = "~0.15.6"
GeometricIntegrators = "~0.16.2"
LaTeXStrings = "~1.4.0"
ODEInterfaceDiffEq = "~3.18.0"
OrdinaryDiffEqFIRK = "~1.19.0"
OrdinaryDiffEqSDIRK = "~1.10.0"
PlutoUI = "~0.7.71"
"""
...
manifest_format = "2.0"
project_hash = "6c889cc8a6ae8225540ac55e4406433db5020717"
...
git-tree-sha1 = "845cd833dea5e3190352888fa8fef624037e6d9a"
...
git-tree-sha1 = "56c6c3ca955d6233de12d6c0ddb22bbd8a5274c1"
...
[[deps.GeometricBase]]
deps = ["Unicode"]
git-tree-sha1 = "b953e001e1dff0e2bce14dc06f7cf96383abec2c"
uuid = "9a0b12b7-583b-4f04-aa1f-d8551b6addc9"
version = "0.14.2"

[[deps.GeometricEquations]]
deps = ["GeometricBase", "Parameters", "Random"]
git-tree-sha1 = "14c89c4fda9200861ad28b3e3523eb26b4baf24c"
uuid = "c85262ba-a08a-430a-b926-d29770767bf2"
version = "0.21.0"

[[deps.GeometricIntegrators]]
deps = ["CompactBasisFunctions", "Documenter", "GenericLinearAlgebra", "GeometricBase", "GeometricEquations", "GeometricIntegratorsBase", "GeometricSolutions", "LinearAlgebra", "OffsetArrays", "Parameters", "PrettyTables", "ProgressMeter", "QuadratureRules", "Reexport", "RungeKutta", "SharedArrays", "SimpleSolvers", "StaticArrays", "Test"]
git-tree-sha1 = "f3f2ccafacd50eb826711900a5aabeb9414d05b0"
uuid = "dcce2d33-59f6-5b8d-9047-0defad88ae06"
version = "0.16.2"

[[deps.GeometricIntegratorsBase]]
deps = ["GeometricBase", "GeometricEquations", "GeometricSolutions", "LinearAlgebra", "OffsetArrays", "Parameters", "SafeTestsets", "SimpleSolvers", "Unicode"]
git-tree-sha1 = "fd13da80b56d7e403b6bcbba19571590c4a8299e"
uuid = "71212ab4-08a2-48eb-a02b-4cbad0e9c44a"
version = "0.2.1"

[[deps.GeometricSolutions]]
deps = ["GeometricBase", "GeometricEquations", "LinearAlgebra", "OffsetArrays", "Test"]
git-tree-sha1 = "bb685c44a7135204f2781eea6b4f42b92e6b3e5c"
uuid = "7843afe4-64f4-4df4-9231-049495c56661"
version = "0.6.4"
...
git-tree-sha1 = "56a3c37af7d93f6234b32b1aa3ad08b2fd143dba"
...
git-tree-sha1 = "5b63a4793a334863eca3c829655dc40413cd4bc7"
uuid = "a08977f5-a20d-4b99-b11f-f5ea535e0575"
version = "0.1.7"

[[deps.QuasiArrays]]
deps = ["ArrayLayouts", "DomainSets", "FillArrays", "LazyArrays", "LinearAlgebra", "StaticArrays"]
git-tree-sha1 = "2d7e5e45c17ce9a9ee9d4ef5e304ed3d82f622a8"
uuid = "c4ea9172-b204-11e9-377d-29865faadc5c"
version = "0.13.5"
weakdeps = ["SparseArrays", "StatsBase"]
...
git-tree-sha1 = "26f6281429062ee4afa0fd50fbde39527046f9da"
...
[[deps.SimpleSolvers]]
deps = ["Distances", "ForwardDiff", "GeometricBase", "LinearAlgebra", "NaNMath", "Printf", "StaticArrays", "Test"]
git-tree-sha1 = "95dc790396162119341bf006bfd6afd6040c0d82"
