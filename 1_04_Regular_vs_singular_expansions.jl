...
md"""
# 1.4 Regular vs. singular expansions

We consider the roots of the quadratic equation

$$\varepsilon x^2 + 2 x - 1 = 0, \qquad 0 < \varepsilon \ll 1.$$
"""

# ╔═╡ 8b2539ae-1078-44fc-9374-684920042c22
md"""
``\varepsilon`` = $(@bind epsilon Slider(
	range(0, 1, length = 101)[2:end],
	default = 1, show_value = true))
"""

# ╔═╡ c7445c04-fd05-402d-bd45-1f5f66f6dc1f
let epsilon = epsilon
	fig = Figure()
	
	ax1 = Axis(fig[1, 1]; xlabel = L"x", ylabel = L"\varepsilon x^2 + 2 x - 1")
	ax2 = Axis(fig[1, 2]; xlabel = L"x")

	f(x) = epsilon * x^2 + 2 * x - 1
	x1 = range(-4 / epsilon, 5.0, length = 500)
	lines!(ax1, x1, f.(x1); label = "quadratic")

	x2 = range(0.35, 0.6, length = 500)
	lines!(ax2, x2, f.(x2))
					   
	for ax in [ax1, ax2]
		scatter!(ax, [0.5], [0.0]; label = L"y_0 = 0")
		scatter!(ax, [0.5 - epsilon / 8], [0.0]; label = L"y_0 + \varepsilon y_1, y_0 = 0")
		scatter!(ax, [-2 / epsilon], [0.0]; label = L"y_0 = -2")
		scatter!(ax, [-2 / epsilon - 1/2], [0.0]; label = L"y_0 + \varepsilon y_1, y_0 = -2")
	end

	axislegend(ax1)
	xlims!(ax2, first(x2), last(x2))
	fig
end
...
manifest_format = "2.0"
project_hash = "02b8255667f793a3dce2c5b9f5a447a2af10203e"
