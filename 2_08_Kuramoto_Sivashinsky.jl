...
# ╔═╡ c7c67925-7fb1-49fb-aa11-4a0c10cf387a
using Printf: @sprintf

# ╔═╡ dc888244-a9e9-4046-8983-b6983190a27f
using LinearAlgebra

# ╔═╡ 8652f333-32a9-4a6f-bba9-3045009c875c
using SummationByPartsOperators

# ╔═╡ e6c64c80-773b-11ef-2379-bf6609137e69
md"""
# 2.8 The Kuramoto-Sivashinsky equation

We consider the Kuramoto-Sivashinsky (KS) equation

$$\begin{equation*}
  \partial_t u(t, x) + \partial_x \frac{u(t,x)^2}{2} + \partial_x^2 u(t, x) + \partial_x^4 u(t,x) = 0,
\end{equation*}$$

with periodic boundary conditions for $x \in [-6 \pi, 6 \pi]$ and initial condition $u(0, x) = \sin(x)$. We discretize it in space using a Fourier collocation method with $2^{10}$ nodes. The resulting ODE is integrated in time using ARS(4,4,3).
"""
...
# ╔═╡ 0d482a02-9c55-4a8b-bfee-8949725e2dd9
begin
	# General interface
...
# ╔═╡ b99c236c-445a-49f2-81b0-a292c05b736b
struct KuramotoSivashinsky <: AbstractEquation end

# ╔═╡ 8442444c-d800-484b-882a-1e44af95b86c
get_u(u, equations::KuramotoSivashinsky) = u

# ╔═╡ 106f9110-2d55-491e-903e-313a20bddceb
...
# ╔═╡ f37083f0-9a95-4fa0-a157-f018dcd6851a
function operator(::typeof(rhs_stiff!), equation::KuramotoSivashinsky, parameters)
	if parameters.D1 isa FourierDerivativeOperator
		0 * I - parameters.D4 - parameters.D2
	else
		-sparse(parameters.D4) - sparse(parameters.D2)
	end
end

# ╔═╡ f86ff33f-b0d0-446e-9115-9098c712b7d9
...
# ╔═╡ 91cc9b4e-a87d-4dc7-93e1-2e54bf3fcc4b
function setup(u_func, equation::KuramotoSivashinsky, tspan,
    ...
    elseif D isa PeriodicDerivativeOperator && isnothing(D2) && isnothing(D4)
        ...
        D2 = D1^2
        D4 = D1^4
    elseif D isa FourierDerivativeOperator && isnothing(D2) && isnothing(D4)
        D1 = D
        D2 = D1^2
        D4 = D1^4
    else
...
# ╔═╡ 47fb3d6d-0827-473e-b136-b83f8e7e38d5
begin
	"""
	    ARS443(T = Float64)
...
# ╔═╡ 3842f36b-5694-4f54-8840-dab5807ccfea
# IMEX ARK solver
...
function solve_imex(rhs_stiff!, rhs_stiff_operator, rhs_nonstiff!,
    ...
    W, factorization, factorizations = let
        ...
        # different factors.
		if W isa SummationByPartsOperators.AbstractPeriodicDerivativeOperator
...
# ╔═╡ 0c4a9816-8ba5-49ac-9c7f-00bdb09f608a
function kuramoto_sivashinsky_video()
    ...
    (; q0, parameters) = setup((t, x, equation) -> sin.(x),
                               equation, tspan, D1)

    fig = Figure(size = (600, 450)) # default size is (600, 450)
    title = Label(fig[0, 1:1], L"Time $t = 0$", tellwidth = false)
    ax1 = Axis(fig[1, 1]; xlabel = L"x", ylabel = L"u(t, x)")
    plt = lines!(ax1, grid(D1), get_u(q0, equation))
    xlims!(ax1, xmin, xmax)
    ylims!(ax1, -3.3, 3.3)
    ...
    filename = joinpath(@__DIR__, "kuramoto_sivashinsky.mp4")
    record(fig, filename; framerate = 30) do io
        function callback(q, parameters, t)
			t_int = round(Int, t)
            if abs(t - t_int) < 0.1 * dt && mod(t_int, 50) == 0
				@info "Running" t
			end
            title.text[] = LaTeXString(@sprintf("Time \$t = %d\$", t_int))
            (; equation) = parameters
            u = get_u(q, equation)
            plt[2] = u
            u_min = min(u_min, minimum(u))
            u_max = max(u_max, maximum(u))
            recordframe!(io)
            return nothing
        end
...
# ╔═╡ 26bb5c9e-f1fd-46d2-a948-e8b1e4fba0e5
video_file = kuramoto_sivashinsky_video();

# ╔═╡ 51d12a2c-686c-435b-83fd-e17a92be3ace
PlutoUI.LocalResource(video_file)
...
manifest_format = "2.0"
project_hash = "02f3792fc21d6786837996795f8b1881a4dff0c7"
