...
# ╔═╡ 0dd30e3a-489c-4525-a9f8-0bd5d01ff139
begin
	using Plots
...
# ╔═╡ 94dbf10f-e6cd-444a-a3d9-b9c4b9febc92
using OrdinaryDiffEqLowOrderRK

# ╔═╡ e744f25f-0826-47d0-93eb-5ec188d5f374
using Trixi

# ╔═╡ 50291450-02d1-4c35-9b08-0b94a1a4a51f
using Printf

# ╔═╡ e6c64c80-773b-11ef-2379-bf6609137e69
md"""
# 3.1 Finite volume method for linear advection in Trixi.jl

First, we consider the linear advection equation

$$\partial_t u + a \partial_x u = 0$$

in 1D with periodic boundary conditions and use a first-order finite volume method available in [Trixi.jl](https://github.com/trixi-framework/Trixi.jl).
"""

# ╔═╡ 751ddd49-89b4-490e-93f3-d18e8b37774e
function initial_condition_smooth(x, t, equations)
	return sinpi(x[1] - equations.advection_velocity[1] * t)
end

# ╔═╡ 6071b868-e58b-428c-ad0d-2e83868c2fc9
function simulate_advection_1d(
	initial_condition;
    t_end            = 10.0,
    cfl              = 0.9,
	alg              = Euler(),
    ...
    # -------------------------------------------------------------------------
    # Trixi setup — linear advection on [-1, 1] with periodic BC
    # -------------------------------------------------------------------------
    advection_velocity = 1.0
    equations = LinearScalarAdvectionEquation1D(advection_velocity)

    # polydeg = 0  →  first-order finite volume
    ...
    # Only the stepsize callback — no summary, no analysis output
    ...
    integrator = init(ode, alg;
                      dt       = 1.0, # overwritten immediately by stepsize_callback
					  adaptive = false,
                      ode_default_options()...,
                      callback = callbacks)

    # -------------------------------------------------------------------------
    # Record mp4 using Plots.jl animation
    # -------------------------------------------------------------------------
    anim = Animation()

    make_frame() = plot(integrator.u, semi;
                        title  = @sprintf("t = %.4f", integrator.t),
                        xlabel = L"x",
                        ylabel = L"u(x,\, t)",
                        xlims  = (coordinates_min, coordinates_max),
                        ylims  = (-1.2, 1.2))

    frame(anim, make_frame())          # t = 0
    while integrator.t < t_end
        step!(integrator)
        frame(anim, make_frame())
    end

    output_file = tempname() * ".mp4"
    mp4(anim, output_file; fps = framerate)
    return output_file
end

# ╔═╡ c39415ee-d30d-4828-9c53-eb623ae95767
md"""
Solving the resulting ODE with an explicit Euler method works well if we use the upwind (local Lax-Friedrichs/Rusanov) flux.
"""

# ╔═╡ a39a7fa6-4fa0-4f52-ad36-e10c96f4cf3c
PlutoUI.LocalResource(simulate_advection_1d(initial_condition_smooth,
	alg = Euler(), surface_flux = flux_lax_friedrichs))

# ╔═╡ 69b4c56a-9717-4c02-b834-044e89376e78
md"""
Switching to the central flux results in problems.
"""

# ╔═╡ a1be8bfe-5496-48d3-9875-07d7b0100295
PlutoUI.LocalResource(simulate_advection_1d(initial_condition_smooth,
	alg = Euler(), surface_flux = flux_central))

# ╔═╡ 9a720d20-dbec-4c4e-a563-4252360f8189
md"""
However, this is an issue of the time integration method, not the spatial semidiscretization.
If we switch to a third-order accurate explicit Runge-Kutta method, the central flux discretization is fine.
"""

# ╔═╡ 5c03bf48-c2fb-414a-a8a0-0849daf166d9
PlutoUI.LocalResource(simulate_advection_1d(initial_condition_smooth,
	alg = BS3(), surface_flux = flux_central))

# ╔═╡ fe3d98d5-c93e-452c-8b37-3eb12d245bf2
md"""
However, with this time integration method, the upwind flux yields worse results.
"""

# ╔═╡ c3585c40-2926-4772-bc53-14e2adae3292
PlutoUI.LocalResource(simulate_advection_1d(initial_condition_smooth,
	alg = BS3(), surface_flux = flux_lax_friedrichs))

# ╔═╡ dbf4277d-766c-4a7c-bb5e-5708e5eb8d2e
md"""
This can be explained by analyzing the dissipation properties of the spatial semidiscretization and the time integration method.
If we reduce the time step size with the explicit Euler method, we see a similar behavior.
"""

# ╔═╡ bd84e8eb-5b80-4c2b-b097-c77ba681c73b
PlutoUI.LocalResource(simulate_advection_1d(initial_condition_smooth,
	alg = Euler(), cfl = 0.1, framerate = 270, surface_flux = flux_lax_friedrichs))

# ╔═╡ a6bafae8-64e9-4192-92f3-7d91fce184cb
md"""
## Discontinuous initial conditions

The example above look like the central flux is strictly better than the upwind flux.
However, this is not the case when the initial date are (close to) discontinuous.

"""

# ╔═╡ 69958972-7c2e-49c9-8b36-0f5bcd7be58e
function initial_condition_discontinuous(x, t, equations)
	if -0.5 < x[1] < 0.5
		return one(x[1])
	else
		return -one(x[1])
...
# ╔═╡ 7b123dc6-5b91-451a-9fa8-297ff67bfb82
PlutoUI.LocalResource(simulate_advection_1d(initial_condition_discontinuous,
	alg = BS3(), surface_flux = flux_central, refinement_level = 6))

# ╔═╡ 10097182-a0f4-442d-b7dc-07a4c349a5b1
PlutoUI.LocalResource(simulate_advection_1d(initial_condition_discontinuous,
	alg = Euler(), surface_flux = flux_lax_friedrichs, refinement_level = 6))

# ╔═╡ 93147000-8156-4fab-a8b5-1100e46173d2
md"""
## 2D version

The behavior is essentially the same in 2D.
"""

# ╔═╡ 73dd14ce-1539-4261-84e2-65b5ee150656
function simulate_advection_2d(;
    t_end            = 10.0,
    cfl              = 0.9,
    alg              = Euler(),
    refinement_level = 5,
    ...
    function initial_condition(x, t, equations)
        vx, vy = equations.advection_velocity
        return sinpi(x[1] - vx * t) * sinpi(x[2] - vy * t)
    end
...
# ╔═╡ fce3e331-483c-41d1-90db-f3c7e74b8c0e
PlutoUI.LocalResource(simulate_advection_2d(
	alg = Euler(), surface_flux = flux_godunov))

# ╔═╡ 08f69aed-eac5-4890-844c-252c357da9a0
PlutoUI.LocalResource(simulate_advection_2d(
	alg = BS3(), surface_flux = flux_central))
...
[compat]
LaTeXStrings = "~1.4.0"
OrdinaryDiffEqLowOrderRK = "~1.13.0"
Plots = "~1.41.6"
PlutoUI = "~0.7.79"
Trixi = "~0.16.6"
"""
...
manifest_format = "2.0"
project_hash = "93f78a81394f342c07f99fa2707014e3903a7b6e"
...
git-tree-sha1 = "abb0b0a6f102fc93d747b5aa4be31c82f15f538c"
...
git-tree-sha1 = "c9cdad2b3fcde3421bc86325457f6c56b22a1aaf"
...
deps = ["ArgCheck", "AutoHashEquals", "FFTW", "InteractiveUtils", "LinearAlgebra", "LoopVectorization", "MuladdMacro", "PolynomialBases", "PrecompileTools", "RecursiveArrayTools", "Reexport", "SciMLBase", "SimpleUnPack", "SparseArrays", "StaticArrayInterface", "StaticArrays", "Unrolled"]
git-tree-sha1 = "5c7e74b0e62ab0855582b2b624ddda4065e30afd"
...
git-tree-sha1 = "64881d3101ec03ad648ead6d7514006ea7218887"
...
[[deps.t8code_jll]]
deps = ["Artifacts", "JLLWrappers", "LazyArtifacts", "Libdl", "MPIABI_jll", "MPICH_jll", "MPIPreferences", "MPItrampoline_jll", "MicrosoftMPI_jll", "OpenMPI_jll", "TOML", "Zlib_jll"]
git-tree-sha1 = "7870d05e3ecaf42e57c3002c7baeae0962d71727"
