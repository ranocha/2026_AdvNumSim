...
# ╔═╡ 903bc947-606e-4301-8408-f24a865fa423
using LinearAlgebra

# ╔═╡ e8326d97-2df5-47f6-92b4-6fea4a388a62
using Theseus # to get coefficients of some IMEX RK methods

# ╔═╡ e6c64c80-773b-11ef-2379-bf6609137e69
md"""
# 2.5 Linear stability analysis of IMEX RK methods

We plot the stability region of the explicit method and the projection of the stability region

$$\begin{equation*}
  S_\mathrm{imex} = \{ \widetilde{z} \in \mathbb{C} \mid |R(\widetilde{z}, z)| \le 1 \text{ for all } z \in \mathbb{C}_- \}.
\end{equation*}$$

First, we see that the projected stability region of the IMEX methods coincides with the stability region of the explicit part for both variants of the IMEX Euler method, i.e., SP(1,1,1) and ARS(1,1,1). 
"""

# ╔═╡ 5dca06a1-da16-4df1-8be1-0273181797de
md"""
However, this is no longer the case for other methods such as higher-order methods.
"""
...
# ╔═╡ 90f3ed65-fa4c-4a85-8f90-219e68a13aee
function evaluate_stability_function_imex(tab; xmin, xmax, ymin, ymax)
	n = 250
	x = range(xmin, xmax, length = n)
	y = range(ymin, ymax, length = n)
	z_ex = @. x + im * y'
	z_im = im .* tan.(range(-pi / 2, pi/2, length = 100))

	vector = z_ex[1] * tab.b_ex + z_im[1] * tab.b_im 
	matrix = I - z_ex[1] * tab.a_ex - z_im[1] * tab.a_im 
	e = ones(eltype(matrix), length(tab.b_ex))
	tmp = similar(e)
	I_s = zero(matrix); copyto!(I_s, I)
	
	stab_ex = zeros(Bool, size(z_ex))
	stab_imex = zeros(Bool, size(z_ex))
	for i in eachindex(stab_imex, z_ex)
		z_ex_i = z_ex[i]

		# explicit part only
		@. vector = z_ex_i * tab.b_ex
		@. matrix = I_s - z_ex_i * tab.a_ex
		fac = lu!(matrix)
		ldiv!(tmp, fac, e)
		R = 1 + transpose(vector) * tmp
		stab_ex[i] = abs2(R) <= 1
		
		# mixed IMEX part
		res = true
		for z_im_i in z_im
			@. vector = z_ex_i * tab.b_ex + z_im_i * tab.b_im 
			@. matrix = I_s - z_ex_i * tab.a_ex - z_im_i * tab.a_im
			fac = lu!(matrix)
			ldiv!(tmp, fac, e)
			R = 1 + transpose(vector) * tmp
			res = res && (abs2(R) <= 1)
	...
	ax_ex = Axis(fig[1, 1]; aspect = DataAspect(), title = "Explicit",
	             xlabel = L"\mathrm{Re}(z)", ylabel = L"\mathrm{Im}(z)")
	heatmap!(ax_ex, x, y, stab_ex, colormap = :binary, colorrange = (0.0, 2.0))
	lines!(ax_ex, [extrema(x)...], [0, 0], color = :black, linestyle = :dash)
	lines!(ax_ex, [0, 0], [extrema(y)...], color = :black, linestyle = :dash)
	
	ax_imex = Axis(fig[1, 2]; aspect = DataAspect(), title = "IMEX",
	               xlabel = L"\mathrm{Re}(z)", ylabel = L"\mathrm{Im}(z)")
	heatmap!(ax_imex, x, y, stab_imex, colormap = :binary, colorrange = (0.0, 2.0))
	lines!(ax_imex, [extrema(x)...], [0, 0], color = :black, linestyle = :dash)
	lines!(ax_imex, [0, 0], [extrema(y)...], color = :black, linestyle = :dash)

	linkyaxes!(ax_ex, ax_imex)
	hideydecorations!(ax_imex)
	
	return fig
end

# ╔═╡ 631fb50f-6449-45e5-bf3e-51d115a60d6c
let alg = Theseus.SP111()
	tab = Theseus.RKTableau(alg, Float64)
	evaluate_stability_function_imex(tab;
									xmin = -2.5, xmax = 0.5,
									ymin = -1.5, ymax = 1.5)
end

# ╔═╡ 9c1a1aa1-4d59-46f7-a970-05bccab93167
let alg = Theseus.ARS111()
	tab = Theseus.RKTableau(alg, Float64)
	evaluate_stability_function_imex(tab;
									xmin = -2.5, xmax = 0.5,
									ymin = -1.5, ymax = 1.5)
...
# ╔═╡ c75b807f-3713-43bd-b84e-75abf0debab2
let alg = Theseus.ARS222()
	tab = Theseus.RKTableau(alg, Float64)
	evaluate_stability_function_imex(tab;
									xmin = -2.5, xmax = 0.5,
									ymin = -2.0, ymax = 2.0)
...
# ╔═╡ aa004a10-65db-442f-b6b1-ea641a0094bf
let alg = Theseus.ARS443()
	tab = Theseus.RKTableau(alg, Float64)
	evaluate_stability_function_imex(tab;
									xmin = -2.5, xmax = 0.5,
									ymin = -2.5, ymax = 2.5)
...
# ╔═╡ ab135c28-7e7b-4ff8-a219-e259319c6e83
let alg = Theseus.SSP2332()
	tab = Theseus.RKTableau(alg, Float64)
	evaluate_stability_function_imex(tab;
									xmin = -5.0, xmax = 0.5,
									ymin = -2.5, ymax = 2.5)
...
manifest_format = "2.0"
project_hash = "e6d5151b054b82abe118db77fd483ef38988edde"
